...
class WindowGenerator():
    ...
    def inverse_transform(self, normalized_data, column_name):
        """Rescale normalized data back to original scale"""
    ...
    def reconstruct_prices(self, model, inputs, test_start_idx=0):
        """Reconstruct full prices = predicted_residual + trend + seasonal"""
        predictions = model(inputs)

        #  residuals (in normalized scale)
        pred_residuals_norm = predictions[0, :, 0].numpy()  # First feature is Close_Residual
...
def main():
    ...
    # keep original components for reconstruction
    ...
    multi_lstm_model_2 = tf.keras.Sequential([
        ...
        tf.keras.layers.LSTM(64, return_sequences=False),  #
        tf.keras.layers.Dropout(0.2),
        tf.keras.layers.RepeatVector(OUT_STEPS),  # Repeat features OUT_STEPS times
    ...
    if RNN:
        multi_lstm_model = simple_rnn_model
    else:
        if not Stacked:
            multi_lstm_model = simple_lstm_model
        else:
            multi_lstm_model = multi_lstm_model_2

    # multi_lstm_model# simple_lstm_model,#
    print(f"\nModel architecture:")
    multi_lstm_model.build(input_shape=(None, input_width, num_features))
    ...
    history = compile_and_fit(multi_lstm_model, multi_window, patience=10, lr=0.0005)
    ...
    # Performance on residuals
    test_performance = multi_lstm_model.evaluate(multi_window.test, verbose=0, return_dict=True)
    ...
    for inputs, labels in multi_window.test:
        predictions = multi_lstm_model(inputs).numpy()

        for i in range(len(predictions)):
            # Get predicted residuals (normalized)
            pred_resid_norm = predictions[i, :, 0]

            # Denormalize
            pred_resid = pred_resid_norm * train_std['Close_Residual'] + train_mean['Close_Residual']

            # Get actual trend and seasonal (need to calculate indices)
    ...
    print(f"  Current LSTM (STL, residuals): {(mae_dollars_residual / 428.0) * 100:.2f}%")
    if len(all_reconstructed) > 0 and len(all_labels) > 0:
        print(f"  Current LSTM (STL, reconstructed): {mape_reconstruction:.2f}%")

    multi_window.plot(multi_lstm_model, rescale=False)

    example_inputs, example_labels = multi_window.example
    example_predictions = multi_lstm_model(example_inputs).numpy()
    ...
    for n in range(3):
        ...
        if base_idx + multi_window.input_width + OUT_STEPS <= len(test_trend):
            # Get trend and seasonal for input window
            ...
            # Get trend and seasonal for prediction window
            ...
            # Input prices (past windowsize days)
            ...
            # Prediction indices (next 5 days)
            ...
            ax.scatter(pred_indices, label_prices, edgecolors='k', label='Actual Future Price',
                       c='green', s=input_width, zorder=5, marker='o')  # maybe s=100

            # Predicted future prices
            ax.scatter(pred_indices, pred_prices, edgecolors='k', label='Predicted Future Price',
                       c='orange', s=input_width, zorder=5, marker='X')  # maybe s=100
    ...
    if len(all_predictions_full) > 0:
        ...
        axes[0].plot(all_dates_full, all_predictions_full, label='LSTM Predicted Price',
                     linewidth=2, color='orange', alpha=0.7, linestyle='--')
        axes[0].set_ylabel('Price ($)', fontsize=12)
        axes[0].set_title('Full Price Reconstruction: LSTM Predictions vs Actual (5-Step Ahead)',
